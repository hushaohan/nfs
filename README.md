...
Imported models are **embedded** as base64 in `js/car_models_data.js`
(generated by `tools/embed-models.js`), so they load identically from
double-clicked `index.html`, GitHub Pages, or any static server — no
fetch, no CORS, no extra requests. Their select cards show a LOADING
